 ...
 * **次の `https?://` の直前でも終わる**（2026-09-24 の訂正。検証の中-1）。ASCII の記号（`,` `|` `(` `)` `"`）で
 * 繋いだ URL を 1 本にまとめると、6 本を 1 本と数えて登録を通し、配信の時刻に Threads が断る。
 * スキームの大文字・小文字を問わない（先読みの `https?://` も `i` フラグで同じく問わない）。
 */
const URL_PATTERN = /https?:\/\/(?:(?!https?:\/\/)[\x21-\x7E])*/gi;
 ...
 *
 * * `https?://` から ASCII の表示文字が続く限り（次の `https?://` の直前まで）を URL とし、末尾の `.,;:!?)]` を外す
 * * スキームだけが残ったものは数えない
