...
**バグ報告と機能提案は Issue から始めてください。**
実装してから「これは Core の責務ではない」と判明すると、書いた分がまるごと無駄になります。
どちらに置くかは §2 で判断できますが、迷ったら先に Issue で相談してください。

### バグ報告 / Bug report

再現できないものは直せません。次を書いてください。

- 何をしたか（手順）
- 何が起きたか / 何が起きてほしかったか
- Torifune のバージョン、Node.js のバージョン、PostgreSQL のバージョン
- 関連するログ（**Secret・トークン・接続文字列を消してから**貼ってください）

### 機能提案 / Feature request

- 解決したい問題（実装案ではなく、困っていること）
- それが Core の責務だと考える理由（§2）
- Plugin として実現できないか検討した結果

**脆弱性は Issue に書かないでください。** §11 の手順に従ってください。
公開の場に書くと、直る前に悪用されます。
...
- **レビューはコードに向けたものであって、書いた人に向けたものではありません。**
- 指摘に同意できないときは、直さずに理由を書いてください。
  黙って直すと、同じ判断が次も繰り返されます。
- 設計上の判断は、コードのコメントか `docs/設計/` に理由を残してください。
  「なぜそうしたか」はレビューのスレッドに埋もれると失われます。

### レビューする側

- **何が問題かではなく、なぜ問題かを書いてください。** 直し方の指示だけでは、
  次に同じ判断をするときの助けになりません。
- 好みと欠陥を区別してください。好みなら、そう明記したうえで任せてください。
- 次を重点的に見てください。
  - §2 の責務境界を越えていないか（Core に入れるべきでないものが入っていないか）
  - §3 のレイヤ責務と依存の向き
  - Permission の検査が Application 層にあるか
  - Secret・トークンがログ・応答・監査ログへ漏れていないか
  - テストが実装より先に書かれ、実装の都合に合わせて書き換えられていないか
...
## 11. 脆弱性の報告 / Reporting a vulnerability
