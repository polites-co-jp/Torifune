...
describe('#11 README', () => {
  ...
  describe.each(PLUGIN_IDS)('#11 %s の README（共通の語）', (id) => {
    ...
    // 2026-09-23 に書き換え（039-social-credential-fields 設計 #45）：既存のアカウントの資格情報は
    // /social の行の「資格情報を設定」で入れ直す・消す。API（PATCH）の手順は別の手段として残す
    // （無料版の間に先に 4 値を入れる場合）。
    it.each(['資格情報を設定', 'PATCH /api/v1/social/accounts/'])('#11 「%s」が現れる', (word) => {
      expect(readme(id)).toContain(word);
    });

    it('#11 汎用の欄の名前「資格情報（アクセストークン等）」が現れない（039 #45）', () => {
      // sns-x-manual の間（credentialFields: []）は汎用の欄が出なくなった。
      expect(readme(id)).not.toContain('資格情報（アクセストークン等）');
    });

    // 039 の後は sns-x-manual の間に資格情報の欄が出ない。それでも「入力欄が無い／出ない」とは書かず、
    // 「資格情報の欄は出ません」「入力は要りません」と書く（039 実装プラン §8 の 10。この検査は残す）。
    it('#11 資格情報の入力欄が「無い」「出ない」と書かない', () => {
