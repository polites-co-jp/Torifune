 ...
 *
 * 待ち時刻（`next_attempt_at`）が消えるのは
 * **予約日時が未来へ変わったときだけ**である（設計 §5.1.1 / §6.2。裁定 #14-a では
 * 飛ばした履歴（`skip_count` / `skip_reason`）は消さず、待ち時刻だけを消す）。
 * 過去の日時のまま保存しても `next_attempt_at` が消えないので、
 * 支度待ちで後ろへ送られた投稿は最大 23 時間待たされる。
 *
 * **過去日時を「いますぐ」と解釈しない**（`2000-01-01` の予約まで即時配信の要求になる）ので、この挙動は変えない。
