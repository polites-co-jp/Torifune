...
| できてしまうこと | 具体的に |
| --- | --- |
| B が登録した投稿を読む | `GET /api/v1/social/posts` で**本文ごと全件返る**（`GET /{id}` も通る） |
| B が登録した投稿を書き換える | `PATCH /api/v1/social/posts/{B の投稿の id}` が通る。配信は **B のアカウントの資格情報**で行われるので、**A は B の SNS アカウントで任意の文章を公開できる** |
| B が登録した投稿を取り消す | `DELETE /api/v1/social/posts/{B の投稿の id}` が通る。配信前に消せる |
| B のアカウントの資格情報を上書きする | `PATCH /api/v1/social/accounts/{B のアカウントの id}` で、**B が登録した資格情報を上書き・削除できる**（**読み出しはできない**） |

つまり、**`social.write` を持つトークンは、他のアプリが登録した投稿の閲覧・書き換え・取り消しと、
