...
* **予約し直しでは数え直さない。** 飛ばされた回数は予約日時を直しても減らない（すぐ下の「予約し直しても…」）
...
> **予約日時が「現在時刻より未来」へ変わったとき**である。先送りでも手前へ引き戻すのでも変わらない。
> **過去の日時のまま保存しても待ち時刻は消えない**ので、最大 23 時間待たされたままになる。
> 「取りやめてから予約し直す」（`status: "draft"` → `status: "scheduled"`）でも同じで、
> 効くのは**予約日時が未来にあること**のほうである。

> **予約し直しても、飛ばされた回数は減らない。**
> 消えるのは**待ち時刻だけ**で、「支度が整っていないのを何回見つけたか」は投稿に残り続ける。
> したがって **2 回飛ばされた投稿は、支度が整わないまま日時を直しても次の 1 回で取りやめ（`failed`）**になる。
> 日時を直したからといって、約 24 時間の猶予が最初から数え直されるわけではない。
>
> **支度（配信 Plugin・資格情報）を整えてから日時を直せば、その時刻に配信される。**
> 一度でも配信に着手できた時点で回数は 0 に戻るので、次に何かで飛ばされたときは再び約 24 時間待てる。
> 支度が整わないまま何度予約し直しても出ないのは、**取りやめを先延ばしにしないための挙動**である。
