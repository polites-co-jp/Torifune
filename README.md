...
> **既存の環境を更新するとき**：マイグレーションでアクセス解析の集計の中身が変わることがあります
> （`020_analytics_breakdown.sql` では `sessions` の定義が変わり、直帰・滞在などの指標が増えました）。
> `pnpm migrate` のあと、**保持している生ログの期間でロールアップを流し直してください**
> （`POST /api/v1/analytics/rollup` に `{"from":"<生ログの最古の日>","to":"<今日>"}`）。
> 手順は [`docs/マニュアル/アクセス解析設置.md`](docs/マニュアル/アクセス解析設置.md) §4-3 にあります。
